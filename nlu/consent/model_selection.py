...
from nlu.consent import ConSent, Config, HYPERPARAMETERS
import nlu.consent.utils as utils
...
class Handler:
    ...
    def test(self,
        ...
        assert isinstance(self.config.load_model, str), \
    ...
    def fine_tune_experiment(self,
        ...
        self.config.dataset_name = dataset_name
        self.config.load_weights = self.config.load_model
        # load test data
        test_data = utils.load_data(test_data_file_path,
                              code_name=self.config.code_name)
        consent_metrics = []
